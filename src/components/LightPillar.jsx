...
const LightPillar = forwardRef(function LightPillar({
  topColor = '#5227FF',
  bottomColor = '#FF9FFC',
  intensity = 1.0,
  rotationSpeed = 0.3,
  ...
  useImperativeHandle(ref, () => ({
    setTargets: (next) => {
      ...
      if (next.intensity !== undefined) {
        materialRef.current.uniforms.uIntensity.value = next.intensity
      }
      if (next.glowAmount !== undefined) {
        materialRef.current.uniforms.uGlowAmount.value = next.glowAmount
      }
    },
  }))
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        float coreDist = length(uv * vec2(0.68, 1.0));
        col *= mix(0.52, 1.0, smoothstep(0.04, 0.82, coreDist));
